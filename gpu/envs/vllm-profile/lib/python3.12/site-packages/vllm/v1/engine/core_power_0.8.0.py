...
class EngineCore:
    ...
    def step(self) -> EngineCoreOutputs:
        ...
        batch_size = len(scheduler_output.scheduled_new_reqs) + len(scheduler_output.scheduled_cached_reqs)

        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        print(f"Time: {current_time}, Batch: {batch_size}, Latency: {token_time:.2f}", end=", ")
        prefill_seqlen = []
        decode_seqlen = []
        # for b in range(len(scheduler_output.scheduled_new_reqs)):
        #     new_req = scheduler_output.scheduled_new_reqs[b]
        #     print(len(new_req.prompt_token_ids), end=", ")
        # print()
        # for b in range(len(scheduler_output.scheduled_cached_reqs)):
        #     new_req = scheduler_output.scheduled_cached_reqs[b]
        #     print(new_req.num_computed_tokens, end=", ")
        # print()
        for b in range(len(scheduler_output.scheduled_new_reqs)):
            prefill_seqlen.append(str(len(scheduler_output.scheduled_new_reqs[b].prompt_token_ids)-1))
        for b in range(len(scheduler_output.scheduled_cached_reqs)):
            decode_seqlen.append(str(scheduler_output.scheduled_cached_reqs[b].num_computed_tokens))
