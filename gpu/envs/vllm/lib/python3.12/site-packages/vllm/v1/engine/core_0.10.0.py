...
class EngineCore:
    ...
    def _execute_model_with_profiling(
        self, scheduler_output: SchedulerOutput
    ) -> ModelRunnerOutput:
        """Execute the model while emitting lightweight latency telemetry."""
        start = time.time()
        model_output = self.model_executor.execute_model(scheduler_output)  # type: ignore
        if scheduler_output.total_num_scheduled_tokens > 0:
            elapsed_ms = (time.time() - start) * 1000.0
            self._log_batch_latency(scheduler_output, elapsed_ms)
        return model_output

    def _await_future_with_profiling(
        self,
        future: Future[ModelRunnerOutput],
        scheduler_output: SchedulerOutput,
        start_time: float,
    ) -> ModelRunnerOutput:
        """Wait for an async model execution and emit latency telemetry."""
    ...
    def _log_batch_latency(self, scheduler_output: SchedulerOutput,
                           latency_ms: float) -> None:
        cached_reqs = scheduler_output.scheduled_cached_reqs
        batch_size = len(scheduler_output.scheduled_new_reqs)
        if cached_reqs is not None:
            batch_size += cached_reqs.num_reqs

        prefill_seqlen = [
            str(max(len(req.prompt_token_ids) - 1, 0))
            for req in scheduler_output.scheduled_new_reqs
        ]
        decode_seqlen: list[str] = []
        if cached_reqs is not None:
            decode_seqlen = [
                str(num) for num in cached_reqs.num_computed_tokens
            ]

        message = (
            f"Prefill: ({' '.join(prefill_seqlen)}), "
            f"Decoding: ({' '.join(decode_seqlen)})"
        )
        print(f"Batch: {batch_size}, Latency: {latency_ms:.2f}", end=", ", flush=True)
    ...
    def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
        ...
        model_output = self.execute_model_with_error_logging(
            self._execute_model_with_profiling,
            scheduler_output)
    ...
    def step_with_batch_queue(
        ...
        if not self.batch_queue.full():
            ...
            if scheduler_output.total_num_scheduled_tokens > 0:
                ...
                self.batch_queue.put_nowait(
                    (future, scheduler_output, start_time))  # type: ignore
        ...
        if not scheduled_batch and not self.batch_queue.empty():
            future, scheduler_output, start_time = self.batch_queue.get_nowait()

            # Blocking until the first result is available.
            model_output = self.execute_model_with_error_logging(
                lambda _: self._await_future_with_profiling(
                    future, scheduler_output, start_time),
                scheduler_output)
